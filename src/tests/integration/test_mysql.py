...
# TODO: move postgreSQLConfig to individual file, can be used by ci-integration-db workflow file
postgreSQLConfig = {
    # Database Config
    "db_name": "db",
...
def get_database_url():
    return "mysql+pymysql://{}:{}@{}/{}".format(postgreSQLConfig["user"], postgreSQLConfig["password"], postgreSQLConfig["endpoint"], postgreSQLConfig["db_name"])
...
@pytest.fixture
def ip_with_db(ip):
    # Disconnect build-in sqlite, make sure we only have one active connection in the %sql
    ip.run_cell(
        "%sql --close sqlite://"
    )
    # Connect
    ip.run_cell(
        "%sql " + get_database_url() + " --alias mySQLTest"
    )
    yield ip
    # Disconnect
    ip.run_cell(
